...
/**
 * Google Veo3 API Interface
 * Provides video generation capabilities using Google's Veo3 model
 */
export interface GoogleVeo3API {
  /**
   * Generate video (unified entry point)
   * @param options - Video generation options
   * @returns Promise with task ID
   */
  generate(options: Veo3GenerateOptions): Promise<Veo3GenerateResponse>;

  /**
   * Generate video from text prompt
   * @param options - Text to video options
   * @returns Promise with task ID
   */
  generateTextToVideo(
   ...
   * @param taskId - Video generation task ID
   * @returns Promise with task details
   */
  getTaskDetails(taskId: string): Promise<Veo3TaskData>;
   ...
   *
   * @param taskId - Video generation task ID
   * @returns Promise with 1080p video URL
   */
  get1080pVideo(taskId: string): Promise<Veo3Task1080pVideoResult>;

  /**
   * Verify and process callback data
   * @param callbackData - Callback data received from webhook
   * @returns Promise with verified task details
   */
  verifyCallback(callbackData: unknown): Promise<Veo3TaskData>;
}
